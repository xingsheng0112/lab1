...
void  OSMutexPend (OS_EVENT *pevent, INT16U timeout, INT8U *err)
{
    ...
    if (CPP_flag == 1){
        CPP_flag = 0;
        // No, Get PIP from mutex            
        pip   = (INT8U)(pevent->OSEventCnt >> 8);
        mprio = (INT8U)(pevent->OSEventCnt & OS_MUTEX_KEEP_LOWER_8);  //     Get priority of mutex owner   
        ptcb  = (OS_TCB *)(pevent->OSEventPtr);                       //     Point to TCB of mutex owner   
        
        if (ptcb->OSTCBPrio != pip && mprio > pip) {  //     Need to promote prio of owner?
            if (pip == 1)
                AddMsgList(OSTimeGet(), 2, ptcb->OSTCBPrio, pip);
            else if (pip == 2)
                AddMsgList(OSTimeGet(), 3, ptcb->OSTCBPrio, pip);
            ...
            // only print when double lock 
            AddMsgList(OSTimeGet(), 3, ptcb->OSTCBPrio, mprio); 
        }
        OS_EXIT_CRITICAL();
    }  
    // here never run
...
INT8U  OSMutexPost (OS_EVENT *pevent)
{
    ...
    if (pip != OS_PRIO_MUTEX_CEIL_DIS) {
        if (OSTCBCur->OSTCBPrio == pip) {               /* Did we have to raise current task's priority? */
            ...
            if (pip == 1)
                AddMsgList(OSTimeGet(), 4, pip, OSTCBCur->OSTCBPrio);
            else if(pip == 2)
                AddMsgList(OSTimeGet(), 5, pip, OSTCBCur->OSTCBPrio);
        }
        else if (OSTCBCur->OSTCBPrio < pip){
            if (pip == 1)
                AddMsgList(OSTimeGet(), 4,  OSTCBCur->OSTCBPrio, OSTCBCur->OSTCBPrio);
            else if (pip == 2)
                AddMsgList(OSTimeGet(), 5,  OSTCBCur->OSTCBPrio, OSTCBCur->OSTCBPrio);
        }
